...
public class TiffToPdfConverter {
    ...
    private static void addImageToPdf(PDDocument document, BufferedImage originalImage, int pageNum) throws Exception {
        ...
        boolean needsRotation = height > width;

        BufferedImage finalImage;
        if (needsRotation) {
            System.out.println("    Page " + pageNum + ": Portrait (" + width + "x" + height + ") -> Rotating 90 degrees");
            finalImage = rotateImage(originalImage, 90);
        } else {
            System.out.println("    Page " + pageNum + ": Landscape (" + width + "x" + height + ")");
